...
{	
    ...
    for (p = 0; p < numPlayer; p++)
    {
			...
			if(G.deckCount[p] != 0)
			{
				for(i = 0; i < (rand() % G.deckCount[p]); i++)
				{
					G.deck[p][i] = copper;
				}
				...
				for(i = 0; i < (rand() % G.handCount[p]); i++)
				{
					G.hand[p][i] = copper;
				}
				...
				myAssert(testG.numActions, G.numActions, "NumActionsTest", p, G, testG, &Ttestcount, &Stestcount, &Ftestcount); //Make sure that the number of actions did not increase
				myAssert(testG.numBuys, G.numBuys, "NumBuysTest", p, G, testG, &Ttestcount, &Stestcount, &Ftestcount); //Make sure that a Buy was not consumed.
				...
				if(TreasureFlag)
				{
					myAssert(testG.handCount[p] , G.handCount[p] * 1 + 2, "Hand Count", p, G, testG, &Ttestcount, &Stestcount, &Ftestcount); // check if the number of cards in our hand increased by 2 for the two treasure cards added.
					if( (testG.hand[p][testG.handCount[p] - 1] == copper || testG.hand[p][testG.handCount[p] - 1] == silver || testG.hand[p][testG.handCount[p] - 1] == gold) && (testG.hand[p][testG.handCount[p] - 2] == copper || testG.hand[p][testG.handCount[p] - 2] == silver || testG.hand[p][testG.handCount[p] - 2] == gold))
						//printf("Treasure cards verified test Passed \n");
						;
					else
						printf("Treasure cards verified test Failure \n");
				}
				 
				myAssert(testG.supplyCount[estate], G.supplyCount[estate] , "Estate supply count verified correct", p, G, testG, &Ttestcount, &Stestcount, &Ftestcount);
				myAssert(testG.supplyCount[duchy], G.supplyCount[duchy] , "duchy supply count verified correct", p, G, testG, &Ttestcount, &Stestcount, &Ftestcount);
				myAssert(testG.supplyCount[province], G.supplyCount[province] , "province supply count verified correct", p, G, testG, &Ttestcount, &Stestcount, &Ftestcount);
				myAssert(testG.supplyCount[copper], G.supplyCount[copper] , "copper supply count verified correct", p, G, testG, &Ttestcount, &Stestcount, &Ftestcount);  //Important to verify that we didn't take these from supply...
				myAssert(testG.supplyCount[silver], G.supplyCount[silver] , "silver supply count verified correct", p, G, testG, &Ttestcount, &Stestcount, &Ftestcount);
				myAssert(testG.supplyCount[gold], G.supplyCount[gold] , "gold supply count verified correct", p, G, testG, &Ttestcount, &Stestcount, &Ftestcount);
