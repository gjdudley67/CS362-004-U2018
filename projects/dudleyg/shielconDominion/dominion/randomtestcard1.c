...
{	
    for (p = 0; p < numPlayer; p++)
    {
				...
				if(G.deckCount[p] + G.discardCount[p] >= 3)
				{
                myAssert(testG.handCount[p] , G.handCount[p] * 1 + 2, "Hand Count", p, G, testG, &Ttestcount, &Stestcount, &Ftestcount); // check if the number of cards in our hand increased by 3 for the three drawn cards. Smithy discards 1 card (itself), so net gain should be 2.
				myAssert(testG.deckCount[p], G.deckCount[p] - 3, "Deck confirmed to have proper card amount remaining", p, G, testG, &Ttestcount, &Stestcount, &Ftestcount);
				}
				
				myAssert(testG.numActions, 1, "NumActionsTest", p, G, testG, &Ttestcount, &Stestcount, &Ftestcount); //Make sure that the number of actions did not increase
