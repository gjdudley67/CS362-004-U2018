 ...
 *
 * CouncilRoomTest: AdventurerTest.c dominion.o rngs.o
 *      gcc -o CouncilRoomTest -g  CouncilRoomTest.c dominion.o rngs.o $(CFLAGS)
 * -----------------------------------------------------------------------
 */
...
int main() {
    ...
    for (p = 0; p < numPlayer; p++)
    {
                ...
                myAssert(testG.handCount[p] , G.handCount[p] * 1 + 3, "Hand Count", p); // check if the number of cards in our hand increased by 4 for the three drawn cards. Council_Room discards 1 card (itself), so net gain should be 3.
				...
				myAssert(testG.deckCount[p] + testG.handCount[p] + testG.discardCount[p] + testG.playedCardCount , 10, "No New Cards Test", p); //This test ensures that no new cards were somehow magically created by the council room card.
				myAssert(testG.numActions, 1, "NumActionsTest", p); //Make sure that the number of actions did not increase
				myAssert(testG.numBuys, 2, "NumBuysTest", p); //Make sure that buy increased by 1.
				...
				if(p == 0)
				{
					myAssert(testG.handCount[1] , 1, "Other Players Gained 1 Card in their hand", p+1); //Make sure the other player didn't somehow gain cards in their hand.
				...
				if(p == 1)
				{
					myAssert(testG.handCount[0] , 6, "Other Players Gained 1 Card in their hand", p-1); //Make sure the other player didn't somehow gain cards in their hand.
