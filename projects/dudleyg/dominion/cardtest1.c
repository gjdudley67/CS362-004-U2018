...
int main() {
    ...
    for (p = 0; p < numPlayer; p++)
    {
                ...
                myAssert(testG.handCount[p] , G.handCount[p] * 1 + 2, "Hand Count", p); // check if the number of cards in our hand increased by 2 for the two treasure cards added.
				...
				myAssert(testG.deckCount[p] + testG.handCount[p] + testG.discardCount[p] + testG.playedCardCount , 10, "No New Cards Test", p); //This test ensures that no new cards were somehow magically created by the adventurer card.
				...
				//Make sure the added cards were coppers in this case, as that is all we could have added in the beginning of the game.
				myAssert(testG.hand[p][testG.handCount[p] - 1], copper, "Copper Added", p);
				myAssert(testG.hand[p][testG.handCount[p] - 2], copper, "2nd Copper Added", p); 
